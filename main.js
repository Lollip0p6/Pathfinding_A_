...
function find(){
    ...
    grid[sx][sy].type = 's';
    grid[fx][fy].type = 'f';
    while (!(cx == fx && cy == fy) && !impossible)
    {
        add_open(grid, h, w, cx, cy, fx, fy, sx, sy);
        impossible = check_min_cost(grid, h, w, fx, fy);
    }
    if (!impossible) {
        final_path(grid, sx, sy, fx, fy);
    }else{
        
        document.getElementById("impossible").innerHTML = "No path found!";      
        reset();
    }
...
function putWall(event){
    ...
    if(draws == "start"){
        ...
        fy = y;
        draws = "";
        document.getElementById("finish").style.width = "50px";
    }else{
        if(grid[x][y].type == 'w'){
            grid[x][y].type = 'g';
        }else{
...
function start(){
    draws = "start";
    document.getElementById("start").style.width = "70px";
    document.getElementById("finish").style.width = "50px";
}

function finish(){
    draws = "finish";
    document.getElementById("finish").style.width = "70px";
    document.getElementById("start").style.width = "50px";
}
